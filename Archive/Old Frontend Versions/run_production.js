...
app.get("/faculty_main/:fac_key", (req, res) => {
  ...
  const validateFacultyQuery = `SELECT *
    FROM Persons
  ...
  db.get(validateFacultyQuery, (err, fac_info) => {
    ...
    if (fac_info) {
      // If validateFacultyQuery was successful...
      ...
      db.all(studentInfoSqlQuery, [], (err, student_info) => {
        ...
        db.all(timeInfoQuery, [], (err, time_info) => {
          if (err) {
            console.log(
              "Error retrieving available meeting times => timeInfoQuery."
            );
            return res.status(500).send("Error retrieving available times");
          }

          if (student_info && time_info) {
            // If studentInfoSqlQuery & timeInfoQuery was successful...
...
app.post("/update-student", (req, res) => {
  const { student_key, first_name, last_name, group, email, fac_key } =
    req.body;

  //Checks the first name field for changes.
  if (first_name != "") {
    //If changes are found, changes are put into query.
    changeFirstNameQuery = `UPDATE Persons
  ...
  if (last_name != "") {
    //If changes are found, changes are put into query.
    changeLastNameQuery = `UPDATE Persons
  ...
  if (group != "") {
    //If changes are found, changes are put into query.
    changeGroupQuery = `UPDATE Persons
      SET "Group" = "${group}"
      WHERE Unique_Key IS "${student_key}"
      `;
    //The query is run to attempt sending the update to the database.
  ...
  if (email != "") {
    //If changes are found, changes are put into query.
    changeEmailQuery = `UPDATE Persons
      SET Email = "${email}"
      WHERE Unique_Key IS "${student_key}"
      `;
    //The query is run to attempt sending the update to the database.
...
app.post("/delete-student", (req, res) => {
  ...
  deleteStudent = `DELETE FROM Persons
    WHERE Unique_Key IS "${student_key}"
  `;
...
app.post("/add-student", (req, res) => {
  ...
  addStudent = `INSERT INTO Persons ("Group", "Last_Name", "First_Name", "Email", "Unique_Key", "Advisor")
...
app.get("/faculty_main/edit_meeting_time/:fac_key/:date/:time", (req, res) => {
  ...
  const meetingQuery = `SELECT * FROM RegistrationList
...
app.post("/update-meeting", (req, res) => {
  ...
  const updateQuery = `UPDATE RegistrationList
...
app.get("/student_main/:student_key", (req, res) => {
  ...
  const validateStudentQuery = `SELECT Unique_Key 
    FROM Persons
    Where "Group" != "Professor" AND Unique_Key = "${req.params.student_key}"
    `;
  //Temp query for retrieving all the meeting times from the database.
  const meetingTimesQuery = `SELECT *
    FROM RegistrationList
    `;
  const currentMeetingQuery = `SELECT *
     FROM RegistrationList
     WHERE Student_ID = "${req.params.student_key}"
    `;
  const checkGroupEligibleQuery = `SELECT Student_ID as sid
     FROM RegistrationList
  ...
  //the database.
  const checkIfRequestedQuery = `SELECT *
     FROM RegistrationList
     WHERE Student_ID = "${req.params.student_key}"
    `;
  //Attempts to request information from the database.
  db.get(validateStudentQuery, (err, student_key) => {
    ...
    if (student_key) {
      // If validateStudentQuery was successful...
      ...
      db.all(studentSqlQuery, [], (err, name_entries) => {
        ...
        db.all(meetingTimesQuery, [], (err, time_info) => {
          if (err) {
            console.log(
              "Error retrieving available meeting times => meetingTimesQuery."
            );
            return res.status(500).send("Error retrieving available times");
          }
          //Attempts to request meeting time information from the database.
          db.all(currentMeetingQuery, [], (err, current_time) => {
            ...
            if (name_entries && time_info) {
              // If the studentSqlQuery, meetingTimesQuery, and studentCurrentMeetingQuery were successful...
              ...
              db.get(checkIfRequestedQuery, (err, time_requested) => {
                ...
                if (err) {
                  console.log(
                    "Error retrieving available meeting times => checkIfRequestedQuery."
                  );
                  return res
                    .status(500)
                    .send("Error retrieving requested time.");
                }
                db.get(checkGroupEligibleQuery, (err, group_tag) => {
                  console.log(group_tag);
                  if (err) {
                    console.log(
                      "Error finding group => checkGroupEligibleQuery."
                    );
...
app.post("/register-time", (req, res) => {
  ...
  const registerQuery = `UPDATE RegistrationList
  ...
  `;
  const findAllRegistrationsQuery = `UPDATE RegistrationList
